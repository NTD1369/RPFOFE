...
@Component({
  selector: 'app-shop-delivery-order-list',
  templateUrl: './shop-delivery-order-list.component.html',
  styleUrls: ['./shop-delivery-order-list.component.scss']
})
export class ShopDeliveryOrderListComponent implements OnInit {

 
  list: TDeliveryHeader [];
  issueId: string;
  lguAdd: string = "Add";
  fromdate = "";
  todate = "";
  ...
  constructor(public authService: AuthService, private deliveryService: DeliveryOrderService, private alertifyService: AlertifyService, private router: Router,
  ...
  //   this.router.navigate(["shop/bills/delivery", 'edit', data.data.transId, data.data.companyCode, data.data.storeId]);
  // }
  delPromotion(data) {
  ...
  viewDetail(data) {
    debugger;
    // this.router.navigate(["shop/grpo", "edit", data.data.purchaseId, data.data.companyCode, data.data.storeId]);
    this.router.navigate(["shop/bills/delivery", 'edit', data.data.transId, data.data.companyCode, data.data.storeId]);
  ...
  loadList() {
    ...
    this.deliveryService.getByType(comp.companyCode, comp.storeId, fromStr, toStr, "","", key, status, this.authService.getCurrentInfor().username).subscribe((response: any) => {
