...
@Component({
  selector: 'app-shop-checkin-menber',
  templateUrl: './shop-checkin-menber.component.html',
  styleUrls: ['./shop-checkin-menber.component.scss']
})
export class ShopCheckinMenberComponent implements OnInit {
  ...
  filterBOM( itemCode) {
    debugger;
    if (this.ItembomList !== null && this.ItembomList !== undefined  && this.ItembomList?.length > 0) {
      let rs = this.ItembomList.filter(x => x.boMid === itemCode);
      return rs;
    }
  ...
  getData(value, type)
  {
    ...
    this.LicensePlate.getMenberInfo(this.authService.getCompanyInfor().companyCode,this.authService.storeSelected().storeId,value,type).subscribe((response: any) => {
          debugger;
          ...
          if (response !== null && response !== undefined) {
            ...
            if (response.success ==true) {
              ...
              this.ItembomList = response.data.vourcherDetailBom;
              console.log(this.ItembomList)
            ...
            else
            {
              ...
              this.order.promoId = "";
                ...
                line.lineTotal = line.price * line.quantity;
                  ...
                  //bom
                  let baseLine = 1;
                  this.ItembomList.forEach(e=>{
                    let bom = this.items.filter(x => x.itemCode ==e.itemCode);
                    if(bom.length >0 && e.boMid == item.itemCode)
                    {
                      baseLine ++;
                    //  bom[0].defaultPrice = item.defaultPrice;
                     let itembomx = bom[0];
                     let itembom =  this.basketService.mapProductItemtoBasket(itembomx,1);
                    
                   
                    let line = new TSalesLine();
                    line.quantity = e.quantity;
                    line.itemCode = itembom.id;
                    line.bomId = item.itemCode;
                    line.price = 0;
                    line.lineTotal = line.price * line.quantity;
                      // debugger;
                      line.uomCode = itembom.uom;
                      line.barCode = itembom.barcode;
                      line.description = itembom.productName;
                      line.storeAreaId = itembom.storeAreaId;
                      line.timeFrameId = itembom.timeFrameId;
                      line.appointmentDate = itembom.appointmentDate;
                      line.promoId = itembom.promotionPromoCode;
                      line.remark = itembom.note;
                      line.baseLine = null;
                      line.baseTransId = item.baseTransId;
                      // line.promoType= item.promotionType;
                      // line.promoPercent= item.promotionDiscountPercent;
                      ...
                      // line.promoLineTotal = item.promotionLineTotal;
                      // line.promoDisAmt = item.promotionDisAmt;
                      line.openQty = itembom.quantity;
                      line.priceListId = itembom.priceListId;
                      line.isPromo = itembom.promotionIsPromo;
                      line.isSerial = itembom.isSerial;
                      line.isVoucher = itembom.isVoucher;
                      line.itemType = itembom.customField1;
                      line.discountType = "Discount Percent";
                      line.discountRate = 0;
                      line.discountAmt = 0;
                      line.lineTotalBefDis = line.lineTotal;
                      line.lineTotalDisIncludeHeader = line.lineTotal;
                      line.status = "C";
                      line.slocId = itembom.slocId;
                      line.prepaidCardNo = itembom.prepaidCardNo;
                      line.memberDate = itembom.memberDate;
                      line.memberValue = itembom.memberValue;
                      line.taxCode = itembom.salesTaxCode;
                      line.taxRate = itembom.salesTaxRate;
                      line.custom1 = this.customerInvoice.customF1;
                      line .custom2 = null;
                      line.custom3 = item.serialNum;
                      // line.taxAmt = line.price * line.quantity * line.discountAmt * line.taxRate;
                  ...
                  this.storePaymentService.getByStore(this.authService.storeSelected().companyCode, this.authService.storeSelected().storeId, terminalId).subscribe((res: any) => {
                    ...
                    if(res.success)
                    {
                      let defautPayment = res.data.filter(x=>x.paymentCode ===itemv.customField10);
                    ...
                    this.basketService.CreateOrderByOrder(this.order).subscribe(
                      async (response: any) => {
                        ...
                        if (response.success) {
                          ...
                          this.getNewCode();
                          this.getData(this.oldvalue,this.oldtype);
