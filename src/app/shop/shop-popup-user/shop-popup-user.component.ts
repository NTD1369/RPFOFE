...
@Component({
  selector: 'app-shop-popup-user',
  templateUrl: './shop-popup-user.component.html',
  styleUrls: ['./shop-popup-user.component.css']
})
export class ShopPopupUserComponent implements OnInit {
  ...
  constructor(private basketService: BasketService, private modalService: BsModalService, private commonService: CommonService, 
    ...
    setTimeout(()=>{
      this.commonService.Shortcuts$.subscribe((data: any)=>{ 
        debugger; 
          ...
          {
            data.forEach(val => newArray.push(Object.assign({}, val))); 
            // this.tempShortcuts = newArray;
            this.commonService.changeTempShortcuts(newArray); 
            console.log('this.tempShortcuts', newArray); 
