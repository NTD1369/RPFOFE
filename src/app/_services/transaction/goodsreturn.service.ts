...
export class GoodsreturnService {
  ...
  getAll(companycode: string, storeid: string, fromdate, todate, key: string, status: string): Observable<TGoodsReceiptPoheader[]> {
    return this.http.get<TGoodsReceiptPoheader[]>(this.baseUrl + 'goodsreturn/GetByType?companyCode='+companycode+'&storeId=' +storeid+'&fromdate=' + fromdate+'&todate=' + todate+'&key=' + key+'&status=' + status);
  }
  getByType(companycode: string, storeid: string, type, fromdate, todate): Observable<TGoodsReceiptPoheader[]> {
    return this.http.get<TGoodsReceiptPoheader[]>(this.baseUrl + 'goodsreturn/GetByType?companyCode='+companycode+'&storeId=' +storeid+'&type=' +type +'&fromdate=' + fromdate+'&todate=' + todate);
  }
  ...
  { 
    return this.http.get(this.baseUrl + 'goodsreturn/getNewNum?companyCode=' + companyCode + '&storeId=' + storeId,  { responseType: 'text' });
  }
  getItemPagedList(page?, itemsPerPage?, userParams?): Observable<PaginatedResult<TGoodsReceiptPoheader[]>> {
    ...
    return this.http.get<TGoodsReceiptPoheader[]>(this.baseUrl + 'goodsreturn/GetPagedList', { observe: 'response', params})
      .pipe(
        map(response => {
  ...
  mapPO2GRPO(po: TPurchaseRequestHeader): TGoodsReceiptPoheader
  {
...
// baseSAPLine: null
  ...
  getBill(id: string, companycode: string, storeid: string): Observable<TGoodsReceiptPoheader> {
    return this.http.get<TGoodsReceiptPoheader>(this.baseUrl + 'goodsreturn/GetOrderById?Id=' + id + '&CompanyCode=' + companycode + '&StoreId='+ storeid);
  }
   
  delete(companyCode: string, storeId: string, Id: string) {
    return this.http.delete(this.baseUrl + 'goodsreturn/delete?companyCode=' + companyCode + '&storeId=' + storeId  + '&Id=' + Id );
  }
  create(invoice: TGoodsReceiptPoheader)
  {
    return this.http.post(this.baseUrl + 'goodsreturn/CreateInvoice', invoice);
  }
  updateStatus(invoice: TGoodsReceiptPoheader)
  {
    return this.http.post(this.baseUrl + 'goodsreturn/UpdateStatus', invoice);
  }
