import {Injectable} from '@angular/core'; 
import {Resolve, Router, ActivatedRouteSnapshot} from '@angular/router';
 
import { Observable, of } from 'rxjs';
import { catchError } from 'rxjs/operators'; 
import { AlertifyService } from '../_services/system/alertify.service';   
import { ReleaseNoteService } from '../_services/common/release-note.service';
import { SReleaseNote } from '../_models/system/releaseNote';
import { MStoreGroup } from '../_models/storegroup';
import { StoregroupService } from '../_services/data/storegroup.service';

@Injectable()

export class StoreGoupListResolver implements Resolve<MStoreGroup[]> {
    pageNumber = 1;
    pageSize = 50;

    constructor(private storeGroupService: StoregroupService, private router: Router,
                private alertify: AlertifyService) {}

    resolve(route: ActivatedRouteSnapshot): Observable<MStoreGroup[]> {
        return this.storeGroupService.getItemPagedList(this.pageNumber, this.pageSize).pipe(
            catchError(error => {
                this.alertify.error('Problem retrieving data');
                this.router.navigate(['/admin']);
                return of(null);
            })
        );
    }
}