...
@Component({
  selector: 'app-management-variant-list',
  templateUrl: './management-variant-list.component.html',
  styleUrls: ['./management-variant-list.component.scss']
})
export class ManagementVariantListComponent implements OnInit {


  dateFormat = "";
  functionId = "Adm_ItemVariant";
  list: MItemVariant[];
  lguAdd: string = "Add";
  promtionType: any;
  ...
  //    });
  // }
  titleApp="Item Variant";
  constructor(public authService: AuthService, private itemVariantService: ItemVariantService,
     private alertifyService: AlertifyService,  private commonService: CommonService , private router: Router) {
      this.customizeText = this.customizeText.bind(this);
    // this.commonService.changeTitleApp(this.titleApp);
  ...
  viewDetail(data) {
    // debugger;
    this.router.navigate(["admin/masterdata/item-variant-setup/", "edit", data.data.variantId]);
  ...
  //   });
  // }
  loadList(validTimeFrom, validTimeTo, status, keyword ) {
       let now = new Date();
    let hf, mf, ht, mt;
    debugger;
    if(validTimeFrom === null || validTimeFrom===undefined) {validTimeFrom = ""}
    ...
    let comp = this.authService.storeSelected().companyCode;
    this.itemVariantService.getAll(validTimeFrom,validTimeTo, status, keyword).subscribe((response: any) => {
      if (response.success == true) {
        // debugger;
