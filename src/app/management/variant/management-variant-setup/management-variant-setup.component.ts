...
import {  MItemVariant, MItemVariantBuy, MItemVariantMap } from 'src/app/_models/itemVariant';
...
import { ItemVariantService } from 'src/app/_services/data/item-variant.service';
...
@Component({
  selector: 'app-management-variant-setup',
  templateUrl: './management-variant-setup.component.html',
  styleUrls: ['./management-variant-setup.component.scss']
})
export class ManagementVariantSetupComponent implements OnInit {
  ...
  itemVariant: MItemVariant;
  header: MItemVariant;
  // customers: SLoyaltyCustomer[] = [];
   
  buyLines: MItemVariantBuy[] = [];
  mapLines: MItemVariantMap[] = [];
  ...
  //   })
  // }
  constructor(public authService: AuthService,   public storeService: StoreService,   public commonService: CommonService,
    ...
    private itemVariantService: ItemVariantService, private router: Router, private route: ActivatedRoute, private itemService: ItemService) {
  
    this.itemVariant = new MItemVariant();
  ...
  // }
  addNewVariant() {
    this.router.navigate(["admin/masterdata/item/variant-setup"]);
  }
  ...
  loyaltyId: string = '';
  mode: string = "";
  functionId ="Adm_ItemVariantSetup";
  ngOnInit() {
    // this.loadRankType();
    // debugger;
    this.showGrid = true;
    ...
    // this.itemVariant.loyaltyCustomers = [];
    this.CustomerSelected = '';
    this.route.params.subscribe(data => {
      debugger;
      this.mode = data['m'];
      this.loyaltyId = data['id'];
  ...
  canUpdate = false;
  bindPromotionToGrid(variant: MItemVariant) {
    debugger;
    // this.buyLines = loyalty.promoBuys;
    this.buyLines = variant.buyList;
    // console.log('loyaltyBuy', loyalty.loyaltyBuy);
    this.mapLines = variant.mapList;
  ...
  loadPromotion() {
    ...
    this.itemVariantService.getItem(this.loyaltyId).subscribe((response: any) => {
      if (response.success === true) {
        debugger
        this.itemVariant = response.data;
  ...
  saveChanges() {
    ...
    if (this.itemVariant.validFrom === null || this.itemVariant.validFrom === undefined || this.itemVariant.validTo === null
       || this.itemVariant.validTo === undefined) {
      this.alertifyService.success('please input from date/to date');
    }
    else {
      // if (this.checkVallid(this.itemVariant.loyaltyType) === true) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          let getlines = [];
          this.mapLines.forEach((item) => { item.lineNum = 0; getlines.push(item) });
          ...
          this.itemVariant.buyList = buylines;
          this.itemVariant.mapList = getlines;
          ...
          // this.itemVariant.validFrom = parseInt(this.leftPad(hF, 2) + this.leftPad(mF, 2));
          // this.itemVariant.validTo = parseInt(this.leftPad(hT, 2) + this.leftPad(mT, 2));
          // if (this.isMon === true) {
          //   this.itemVariant.isMon = 'Y';
          ...
          if (this.mode === "copy") {
            this.itemVariant.variantId = "";
          }
          if (this.mode === "setup") {
            debugger
            let XDate= new Date(this.itemVariant.validFrom.toString());
            let YDate= new Date(this.itemVariant.validTo.toString());
            let bT = this.GetDateFormat(XDate);
            let bF = this.GetDateFormat(YDate);
            this.itemVariant.validFrom = bT;
            this.itemVariant.validTo = bF;
          }
          let storeNum = 1;
          ...
          this.itemVariantService.create(this.itemVariant).subscribe((response: any) => {
            if (response.success) {
              //  debugger;
              this.alertifyService.success('save completed successfully. ' + response.message);

              this.router.navigate(["admin/crm/edit", response.data]);
            }
            else {
  ...
  onGroupBuySelectionChanged(selectedRowKeys, cellInfo, event, e, dropDownBoxComponent) {
    ...
    if (selectedRowKeys.length > 0) {
      ...
      // mchier: string="";
      // mcname: string="";
      let code = event.selectedRowsData[0].mcid;
      let name = event.selectedRowsData[0].mcname;
      let uom = '';
      // // this.dataGrid.instance.cellValue(cellInfo.rowIndex, 'keyId', selectedRowKeys[0]); 
      this.buytt.instance.cellValue(cellInfo.rowIndex, 'lineCode', code);
      this.buytt.instance.cellValue(cellInfo.rowIndex, 'lineName', name);
