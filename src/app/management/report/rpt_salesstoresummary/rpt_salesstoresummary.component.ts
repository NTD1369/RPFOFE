...
import { RPT_SalesStoreSummaryModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt_salesstoresummary',
  templateUrl: './rpt_salesstoresummary.component.html',
  styleUrls: ['./rpt_salesstoresummary.component.scss']
})
export class Rpt_salesstoresummaryComponent implements OnInit {

  isLoadingData = false;
  list: RPT_SalesStoreSummaryModel[]=[];  
  isNew:boolean = false;
  constructor(private reportService: ReportService, private alertify: AlertifyService, private router: Router,public authService: AuthService,
    private controlService: ControlService, private modalService: BsModalService, private route: ActivatedRoute,public translate: TranslateService) {
  ...
  controlList: any[];
  functionId = "RPT_SalesStoreSummary";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.get_RPT_SalesStoreSummary(companycode, '', user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
      // debugger;
  ...
  {
    this.exportExcel(this.authService.storeSelected().companyCode,this.authService.storeSelected().storeId, this.authService.getCurrentInfor().username,from, to);
  }
  ...
  this.controlList.forEach(item=>{
    ...
    let nameOfRpt= 'SalesStoreSummary' ;
    var d = new Date();
    let dateFm = this.GetDateFormat(d);
    ...
    this.reportService.export_RPT_SalesStoreSummary(companycode, '', user, fromvl, tovl,header).subscribe((res:any) => {
      // loadItems
      // debugger;
      this.isLoadingData = false;
