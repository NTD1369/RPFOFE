...
import { RPT_SalesBySalesPersonModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt_sales-by-sales-person',
  templateUrl: './rpt_sales-by-sales-person.component.html',
  styleUrls: ['./rpt_sales-by-sales-person.component.scss']
})
export class Rpt_salesBySalesPersonComponent implements OnInit {


  isLoadingData = false;
  list: RPT_SalesBySalesPersonModel[]=[];  
  isNew:boolean = false;
  ...
  controlList: any[];
  functionId = "RPT_SalesBySalesPerson";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.Get_RPT_SalesBySalesPerson(companycode, '', user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
