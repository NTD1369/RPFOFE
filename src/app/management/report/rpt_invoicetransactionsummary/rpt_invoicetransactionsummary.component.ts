...
import { RPT_InvoiceTransactionSummaryModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt_invoicetransactionsummary',
  templateUrl: './rpt_invoicetransactionsummary.component.html',
  styleUrls: ['./rpt_invoicetransactionsummary.component.css']
})
export class Rpt_invoicetransactionsummaryComponent implements OnInit {

  functionId = "RPT_InvoiceTransactionSummary";
  marked = false;
  userParams: any = {};
  totalBill=0;
  exchangeBill=0;
  ...
  list: RPT_InvoiceTransactionSummaryModel[]=[]; 
  isNew:boolean = false;
  ...
  onExporting(e) {
    ...
    let nameOfRpt= 'InvoiceTransactionSummary' ;
    var d = new Date();
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.get_RPT_InvoiceTransactionSummary(companycode, '', user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
      // debugger;
