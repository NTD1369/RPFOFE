...
import { RPT_InvoiceTransactionDetailModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt_invoicetransactiondetail',
  templateUrl: './rpt_invoicetransactiondetail.component.html',
  styleUrls: ['./rpt_invoicetransactiondetail.component.css']
})
export class Rpt_invoicetransactiondetailComponent implements OnInit {

  isLoadingData = false;
  list: RPT_InvoiceTransactionDetailModel[]=[]; 
  isNew:boolean = false;
  constructor(private reportService: ReportService, private alertify: AlertifyService, private router: Router, public authService: AuthService,
    ...
    onExporting(e) {
      ...
      let nameOfRpt= 'InvoiceTransactionDetail' ;
      var d = new Date();
    ...
    controlList: any[];
    functionId = "RPT_InvoiceTransactionDetail";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.get_RPT_InvoiceTransactionDetail(companycode, '', user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
      debugger;
