...
import { RPT_InventorySerialModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt-inventory-serial',
  templateUrl: './rpt-inventory-serial.component.html',
  styleUrls: ['./rpt-inventory-serial.component.css']
})
export class RptInventorySerialComponent implements OnInit {

  isLoadingData = false;
  list: RPT_InventorySerialModel []=[];  
  isNew:boolean = false;
  ...
  fromDate: Date;
    ...
    controlList: any[];
    functionId = "USP_RPT_InventorySerial";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.get_RPT_InventorySerial(companycode, this.authService.storeSelected().storeId, user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
