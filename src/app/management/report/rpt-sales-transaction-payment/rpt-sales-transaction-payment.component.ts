...
import { RPT_SalesByYearModel, RPT_SalesTransactionPaymentModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt-sales-transaction-payment',
  templateUrl: './rpt-sales-transaction-payment.component.html',
  styleUrls: ['./rpt-sales-transaction-payment.component.scss']
})
export class RptSalesTransactionPaymentComponent implements OnInit {

  isLoadingData= false;
  list: RPT_SalesTransactionPaymentModel []=[];  
  isNew:boolean = false;
  storeOptions:any =[];
  constructor(private reportService: ReportService, private alertify: AlertifyService, private router: Router,public authService: AuthService,public translate: TranslateService,
  ...
  onExporting(e) {
    ...
    let nameOfRpt= 'SalesTransactionPayment' ;
    var d = new Date();
  ...
  fromDate: Date;
    ...
    controlList: any[];
    functionId = "RPT_SalesTransactionPayment";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.Get_RPT_SalesTransactionPayment(companycode, store, user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
...
this.controlList.forEach(item=>{
  ...
  this.reportService.export_RPT_SalesTransactionPayment(companycode, store, user, fromvl, tovl,header).subscribe((res:any) => {
    // loadItems
    // debugger;
    this.isLoadingData = false;
