...
import { RPT_SalesByHourModel } from 'src/app/_models/common/report';
...
@Component({
  selector: 'app-rpt-sales-by-hour',
  templateUrl: './rpt-sales-by-hour.component.html',
  styleUrls: ['./rpt-sales-by-hour.component.scss']
})
export class RptSalesByHourComponent implements OnInit {


  isLoadingData= false;
  list: RPT_SalesByHourModel[]=[];  
  isNew:boolean = false;
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    this.reportService.get_RPT_SalesByHour(companycode, '', user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
