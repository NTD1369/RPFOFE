...
@Component({
  selector: 'app-rpt-store-listing',
  templateUrl: './rpt-store-listing.component.html',
  styleUrls: ['./rpt-store-listing.component.scss']
})
export class RptStoreListingComponent implements OnInit {
  ...
  controlList: any[];
  functionId = "RPT_SYNC_LISTING_CMP";
  ...
  loadItems(companycode,  from, to, FItem, TItem) {
    ...
    this.reportService.Get_RPT_SYNC_LISTING_CMP(companycode, FItem, TItem, fromvl, tovl).subscribe((res:any) => {
      // loadItems
      // debugger;
      ...
      if(res.success)
      {
        if(res.message?.length > 0)
        {
          ...
          Swal.fire({
            icon: 'warning',
            title:'Report Sync Item Store Listing',
            text:  res.message
          });
        }
