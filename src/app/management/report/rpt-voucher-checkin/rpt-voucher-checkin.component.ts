...
@Component({
  selector: 'app-rpt-voucher-checkin',
  templateUrl: './rpt-voucher-checkin.component.html',
  styleUrls: ['./rpt-voucher-checkin.component.css']
})
export class RptVoucherCheckinComponent implements OnInit {
  ...
  loadItems(companycode, store, user, from, to, keyword) {
    debugger;
    ...
    this.reportService.Get_RPT_VoucherCheckIn(companycode, store, user, fromvl, tovl, keyword).subscribe((res:any) => {
      // loadItems
     ...
     // debugger;
      if(response.success)
      {
       response.data.forEach(item => {
         let data = {name: item.storeId + ' - ' + item.storeName, value:item.storeId  }
