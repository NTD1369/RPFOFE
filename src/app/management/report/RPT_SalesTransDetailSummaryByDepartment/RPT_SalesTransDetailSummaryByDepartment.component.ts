...
@Component({
  selector: 'app-RPT_SalesTransDetailSummaryByDepartment',
  templateUrl: './RPT_SalesTransDetailSummaryByDepartment.component.html',
  styleUrls: ['./RPT_SalesTransDetailSummaryByDepartment.component.scss']
})
export class RPT_SalesTransDetailSummaryByDepartmentComponent implements OnInit {
  ...
  isNew:boolean = false;
  storeOptions:any =[];
  title = "Sales Report Summary By Department";
  ...
  //   let stores = Array.from(new Set(this.list.map((item: any) => item.StoreId)))
  //   debugger;
  //    for ( let  index = 0; index < stores.length; index++) {
  //     let element = stores[index];
  //     debugger;
  //     let StoreNameEXX = "";
  //     let Store: MStore  = this.storeLoaded.find(x=>x.storeId === element);
  //     headerStoreName = Store.storeId + " - " + Store.storeName;
  //     addStore = Store.address;
  //     telStore = Store.phone;
  //     StoreNameEXX = Store.storeId;
    
  //     let headerBy = infor.username; 
  //     let headerDate = this.GetDateFormat(new Date(), true) ; 
  ...
  //         pdfDoc.setFontSize(12); 
  //         pdfDoc.text(headerStoreName,  width/2, 20 , { align: 'center' });   
  //         pdfDoc.setFontSize(9);
  //         pdfDoc.text(addStore,  width/2, 35 , { align: 'center' });   
  //         pdfDoc.text("TEL: " + telStore,  width/2, 45 , { align: 'center' });   
  //         pdfDoc.setFontSize(15);
  //         pdfDoc.text(this.rpTitle,  width/2, 60 , { align: 'center' });   
  //         pdfDoc.setFontSize(12);
  //         pdfDoc.text("From: " + this.rpRangeFr + " To: " + this.rpRangeTo,  width/2, 76 , { align: 'center' });   
   
  //         pdfDoc.setFontSize(9);
          
  //         pdfDoc.text("Print By: " + headerBy, 40, 80, { baseline: 'top' });
  //         pdfDoc.text("Print Date: " + headerDate, 40, 90, { baseline: 'top' });
  //         mrTop = 120;
  //       }
  ...
  //     this.dataGrid.instance.getDataSource().filter([['StoreId','=', element] ]);
  //     const options = {
  ...
  //         }
  //         setTimeout(() => {
  //           this.dataGrid.instance.getDataSource().filter(null);
  //           pdfDoc.save(StoreNameEXX + "_"+ this.rpTitle+'.pdf');
  ...
  //       numOfExport = 1000;
  //     }
  //     await timer(numOfExport).pipe(take(1)).toPromise(); 
  //   }
    ...
    loadDashboard()
  {
    ...
    controlList: any[];
    functionId = "RPT_SalesTransDetailSummaryByDepartment";
  ...
  GetDateFormat(date, includeTime?) {
    var month = (date.getMonth() + 1).toString();
    month = month.length > 1 ? month : '0' + month;
    var day = date.getDate().toString();
    day = day.length > 1 ? day : '0' + day;
    let result =date.getFullYear()+ '-' +  month+ '-' + (day) ;
    if(includeTime!==null && includeTime!==undefined && includeTime)
    {
      let hour = date.getHours().toString()??"0";
      hour = hour.length > 1 ? hour : '0' + hour;
      let min = date.getMinutes().toString()??"0"; 
      min = min.length > 1 ? min : '0' + min;
      let sec = date.getSeconds().toString()??"0";
  ...
  loadItems(companycode, store, user, from, to) {
    ...
    {
      this.rpStoreIdOnely = store;
    }
    this.rpRangeFr = fromvl; 
    this.rpRangeTo = tovl; 
    this.rpTitle = "Sales Report Summary By Department";
    ...
    this.reportService.Get_RPT_SalesTransactionSummaryByDepartment(companycode, store, user, fromvl, tovl).subscribe((res:any) => {
      // loadItems
      // debugger;
  ...
  loadStore(){
    ...
    this.storeService.getByUserWithStatus(this.authService.decodeToken?.unique_name).subscribe((response: any) => {
     // debugger;
      if(response.success)
      {
        this.storeLoaded = response.data ;
