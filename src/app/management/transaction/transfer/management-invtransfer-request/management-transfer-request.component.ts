...
@Component({
  selector: 'app-management-transfer-request',
  templateUrl: './management-transfer-request.component.html',
  styleUrls: ['./management-transfer-request.component.scss']
})
export class ManagementTransferRequestComponent implements OnInit {
  ...
  loadList() {
   ...
   if(this.production===false)
   {
    ...
    this.inventory.getInventoryList(store.companyCode,'', '', 'Q', status, fromStr, toStr, key, user).subscribe((response: any) => {
      if (response.success) {
        ...
        this.inventoryList = response.data.filter(x=>x.fromStore===store.storeId || x.toStore ===store.storeId);
  ...
  newModel() {
    ...
    this.router.navigate(["admin/inventory/transfer-request", 'i', '']);
  }
  viewDetail(data: any) {
    debugger;
    this.router.navigate(["admin/inventory/transfer-request", 'e', data.invtid]);
  }
  print(data) {
    this.router.navigate(["admin/print/transfer-request/", data.invtid]);
  }
