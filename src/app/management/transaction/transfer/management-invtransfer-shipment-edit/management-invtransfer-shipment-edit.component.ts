...
@Component({
  selector: 'app-management-invtransfer-shipment-edit',
  templateUrl: './management-invtransfer-shipment-edit.component.html',
  styleUrls: ['./management-invtransfer-shipment-edit.component.scss'],
  providers: [DatePipe] // Add this
})
export class ManagementInvtransferShipmentEditComponent implements OnInit {
  ...
  constructor(public authService: AuthService, private inventoryService: InventoryService,private datePipe: DatePipe, private controlService: ControlService,
    private whsService: WarehouseService, private storeService: StoreService, private routeNav: Router, private spinnerService: NgxSpinnerService, private formBuilder: FormBuilder,
    ...
    private printService: PrintService, private router: Router, private excelSrv: ExcelService, public commonService: CommonService, private permissionService: PermissionService) {
  ...
  loadToStore() {
    ...
    this.storeService.getAll(this.authService.getCurrentInfor().companyCode).subscribe((response: any) => {
      if (response.success) {
        this.toStoreList = response.data.filter(s => s.status === 'A' && s.storeId !== store);
        ...
        if (arrByWhs.length > 0) {
          ...
          if (this.countRow > 0) {
            for (var i = 0; i < this.countRow; i++) {
              this.dataGrid.instance.cellValue(i, 'From Bins', this.arrWhsFrm[0]);
            }
          }
  ...
  ngOnInit() {
    ...
    if (this.mode === 'e') {
      ...
      if(this.invId !='' && this.invId != undefined && this.invId!= null)
        {
          this.transferFromResquest(this.invId);
          this.isEdit = false;
        }
    }
  ...
  saveModel() {
    ...
    if(canAdd)
    {
      ...
      if(check)
      {
        let itemCodeCheck = this.lines.filter(x=> x.itemCode===undefined || x.itemCode === null || x.itemCode?.length<=0);
        if(itemCodeCheck!==null && itemCodeCheck!==undefined && itemCodeCheck?.length > 0)
        {
          check = false;
        }
      }
              ...
              if (this.isNew === true) {
                ...
                this.inventoryService.create(this.inventory).subscribe((response: any) => {
                  ...
                  if (response.success === true) {
                    ...
                    setTimeout(() => {
                      this.routeNav.navigate(["admin/inventory/transfer-shipment", 'e', response.message]).then(() => { window.location.reload(); });
                    }, 50);
      ...
      else
      {
        Swal.fire({
          icon: 'warning',
          title: 'Item In Data List',
          text: "Please check data in grid (Item Code, Validate data)"
        });
      }
    }
  }
  ...
  onFileChange(evt: any, template) {
    ...
    reader.onload = (e: any) => {
      ...
      console.log(this.importContent,template);
  ...
  downloadTemplate() {
    setTimeout(() => {
      this.commonService.download('Transfer_ShipmentTemlate.xlsx');
    }, 2);
  ...
  exportExcel() {
    ...
    let day = new Date()
    let fileName = "TransferShipMent" + day.toLocaleString();
  ...
  CancelModel() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.inventory.companyCode = this.store.companyCode;
        this.inventory.docType = "S";
        this.inventory.name = "Hủy " + this.inventory.invtid
        this.inventory.fromStore = this.fromStore.storeId;
        this.inventory.fromStoreName = this.fromStore.storeName;
        this.inventory.toStore = this.toStore.storeId;
        this.inventory.toStoreName = this.toStore.storeName;
        ...
        this.inventoryService.create(this.inventory).subscribe((response: any) => {
          if (response.success === true) {
            this.alertifyService.success("Create successfully completed. Trans Id: " + response.message);
            this.routeNav.navigate(["admin/inventory/transfer-shipment", 'e', response.message]).then(()=>{
              window.location.reload();
            });
          }
          else {
  ...
  transferFromResquest(invtid) {
    ...
    this.inventoryService.getInventoryTransfer(invtid, this.store.storeId, this.store.companyCode).subscribe((response: any) => {
      setTimeout(() => {
        ...
        debugger;
        this.lines = response.data.lines.filter(x=>x.approve!==null && x.approve !==0);
        this.lines.forEach(line => {
          line.quantity = line.approve;
          line.baseTransId = line.invtid;
        });
        this.inventory.refInvtid = this.inventory.invtid;
        this.loadSlocFromStore(this.inventory.fromStore);
        this.loadSlocToStore(this.inventory.toStore);
