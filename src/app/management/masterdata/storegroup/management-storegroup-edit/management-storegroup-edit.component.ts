...
@Component({
  selector: 'app-management-storegroup-edit',
  templateUrl: './management-storegroup-edit.component.html',
  styleUrls: ['./management-storegroup-edit.component.scss']
})
export class ManagementStoregroupEditComponent implements OnInit {

  @Input() model: MStoreGroup;
  ...
  controlList: any[];
  groupControlList = {};
  constructor(private storeGroupService: StoregroupService, private authService: AuthService,
  ...
  ngOnInit() {
    this.controlService.getControlByFunction(this.authService.getCurrentInfor().companyCode, this.functionId).subscribe((response: any) => {
      if (response.data.length > 0) {
        ...
        // if (this.model.storeGroupId === null || this.model.storeGroupId === "" || this.model.storeGroupId === undefined) {
        //   this.isNew = true;
        // }
