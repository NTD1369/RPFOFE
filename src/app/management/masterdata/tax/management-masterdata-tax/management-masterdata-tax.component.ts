...
@Component({
  selector: 'app-management-masterdata-tax',
  templateUrl: './management-masterdata-tax.component.html',
  styleUrls: ['./management-masterdata-tax.component.scss']
})
export class ManagementMasterdataTaxComponent implements OnInit {
  ...
  userParams: any = {};
  isNew:boolean = false;
  constructor(private taxService: TaxService, private alertify: AlertifyService, private router: Router, private authService: AuthService,
  ...
  modalRef: BsModalRef;
  tax: MTax;
  openModal(isNew: boolean, tax: MTax, template: TemplateRef<any>) {
    debugger;
    this.isNew = isNew;
    if(isNew)
    {
      this.tax = new MTax();
      this.tax.companyCode = this.authService.getCurrentInfor().companyCode;
    }
    else
    {
      this.tax = tax;
  ...
  updateModel(model) {
    ...
    if(this.isNew)
    {
      ...
      this.taxService.create(model).subscribe((response: any) => {
        if(response.success)
        {
      ...
      this.taxService.update(model).subscribe((response: any) => {
        if(response.success)
        {
