...
@Component({
  selector: 'app-management-masterdata-storage',
  templateUrl: './management-masterdata-storage.component.html',
  styleUrls: ['./management-masterdata-storage.component.scss']
})
export class ManagementMasterdataStorageComponent implements OnInit {


  functionId = "Adm_Storage";
  items: MStorage[];
  ...
  constructor(private storageService: StorageService, private alertify: AlertifyService, private router: Router, private authService: AuthService,
  ...
  updateModel(model) {
    ...
    if (this.isNew) {
      ...
      this.storageService.create(model).subscribe((response: any) => {
        if (response.success) {
      ...
      this.storageService.update(model).subscribe((response: any) => {
        if (response.success) {
