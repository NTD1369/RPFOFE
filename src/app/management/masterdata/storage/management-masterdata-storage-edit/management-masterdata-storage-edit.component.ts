...
@Component({
  selector: 'app-management-masterdata-storage-edit',
  templateUrl: './management-masterdata-storage-edit.component.html',
  styleUrls: ['./management-masterdata-storage-edit.component.scss']
})
export class ManagementMasterdataStorageEditComponent implements OnInit {

  @Input() model: MStorage;
      ...
      if (response.data.length > 0) {
        ...
        response.data.forEach(item => {
          ...
          if (item.controlType === "DateTime") {
            ...
            if (item.controlType.toLowerCase() === "checkbox") {
              debugger;
              let value = this.model[item.controlId];
              if (value !== true) {
                value = false;
              }
              // this.model[item.controlId] = value;
              // group[item.controlId] = [this.model[item.controlId] , ];
              group[item.controlId] = new FormControl({ value: value, disabled: false })
              //group["createdOn"] = new FormControl({ value: date, disabled: false })
              // this.model[item.controlId].setDate(this.model[item.controlId]);
              // console.log(this.model[item.controlId]);
