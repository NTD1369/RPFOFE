...
@Component({
  selector: 'app-management-import-paymentmethod',
  templateUrl: './management-import-paymentmethod.component.html',
  styleUrls: ['./management-import-paymentmethod.component.scss']
})
export class ManagementImportPaymentmethodComponent implements OnInit {

  
  value: any[] = [];
  importContent: MPaymentMethod[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private paymentmethodService: PaymentmethodService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_PaymentMethod.xlsx');
  }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MPaymentMethod());
  ...
  onImportData()
  {
    ...
    data.paymentMethod  = this.importContent;
    
    this.paymentmethodService.import(data).subscribe((response: any)=>{
