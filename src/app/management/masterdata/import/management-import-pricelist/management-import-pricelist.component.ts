...
@Component({
  selector: 'app-management-import-pricelist',
  templateUrl: './management-import-pricelist.component.html',
  styleUrls: ['./management-import-pricelist.component.scss']
})
export class ManagementImportPricelistComponent implements OnInit {

 
  value: any[] = [];
  importContent: MPriceList[] = [];
  isResult= false;
  constructor( private commonService: CommonService ,private priclistService: PricelistService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
    downloadTemplate()
    {
      this.commonService.download('M_PriceList.xlsx');
    }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MPriceList());
          ...
          for(let j = 0; j < header.length; j++)
          {
            ...
            // debugger;
             if(ki.toLowerCase() ===kj.toLowerCase())
             {
              ...
              if(ki.toLowerCase() === 'validfrom' || ki.toLowerCase() === 'validto')
              {
                debugger;
  ...
  onImportData()
  {
    ...
    data.priceList  = this.importContent;
    
    this.priclistService.import(data).subscribe((response: any)=>{
