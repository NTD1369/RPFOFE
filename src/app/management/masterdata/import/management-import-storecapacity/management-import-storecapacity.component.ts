...
@Component({
  selector: 'app-management-import-storecapacity',
  templateUrl: './management-import-storecapacity.component.html',
  styleUrls: ['./management-import-storecapacity.component.scss']
})
export class ManagementImportStorecapacityComponent implements OnInit {

  
  value: any[] = [];
  importContent: MStoreCapacity[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private storecapacityService: CapacityService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_StoreCapacity.xlsx');
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MStoreCapacity());
  ...
  onImportData()
  {
    ...
    data.storeCapacity  = this.importContent;
    
    this.storecapacityService.import(data).subscribe((response: any)=>{
