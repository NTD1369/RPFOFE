...
@Component({
  selector: 'app-management-import-itemgroup',
  templateUrl: './management-import-itemgroup.component.html',
  styleUrls: ['./management-import-itemgroup.component.scss']
})
export class ManagementImportItemgroupComponent implements OnInit {

  value: any[] = [];
  importContent: MItemGroup[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private itemgroupService: ItemgroupService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }

  downloadTemplate()
  {
    this.commonService.download('M_ItemGroup.xlsx');
  }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MItemGroup());
  ...
  onImportData()
  {
    ...
    data.itemGroup  = this.importContent;
    
    this.itemgroupService.import(data).subscribe((response: any)=>{
