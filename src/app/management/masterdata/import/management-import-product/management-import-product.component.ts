...
import { ProductService } from 'src/app/_services/data/product.service';
import { AlertifyService } from 'src/app/_services/system/alertify.service';

@Component({
  selector: 'app-management-import-product',
  templateUrl: './management-import-product.component.html',
  styleUrls: ['./management-import-product.component.scss']
})
export class ManagementImportProductComponent implements OnInit {

  
  value: any[] = [];
  importContent: MProduct[] = [];
  isResult= false;
  constructor(
    private commonService: CommonService ,private productService: ProductService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
 
  ngOnInit() {
  }
  downloadTemplate()
  {
    this.commonService.download('M_Product.xlsx');
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MProduct());
  ...
  onImportData()
  {
    ...
    data.product  = this.importContent;
    
    this.productService.import(data).subscribe((response: any)=>{
