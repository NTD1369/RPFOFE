...
@Component({
  selector: 'app-management-import-itemserial',
  templateUrl: './management-import-itemserial.component.html',
  styleUrls: ['./management-import-itemserial.component.scss']
})
export class ManagementImportItemserialComponent implements OnInit {

 
  value: any[] = [];
  importContent: MItemSerial[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private itemSerialService: ItemserialService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_ItemSerial.xlsx');
  }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MItemSerial());
        ...
        for (let i = 0; i < excelHeader.length; i++) { 
          for(let j = 0; j < header.length; j++)
          {
            ...
            // debugger;
             ...
             {
              if(ki.toLowerCase() === 'expDate' || ki.toLowerCase() === 'expdate' || ki.toLowerCase() === 'storeddate' || ki.toLowerCase() === 'storedDate')
              {
                debugger;
  ...
  onImportData()
  {
    ...
    data.itemSerial  = this.importContent;
    
    this.itemSerialService.import(data).subscribe((response: any)=>{
