...
@Component({
  selector: 'app-management-import-storearea',
  templateUrl: './management-import-storearea.component.html',
  styleUrls: ['./management-import-storearea.component.scss']
})
export class ManagementImportStoreareaComponent implements OnInit {

  
  value: any[] = [];
  importContent: MStoreArea[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private storeareaService: StoreareaService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_StoreArea.xlsx');
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MStoreArea());
  ...
  onImportData()
  {
    ...
    data.storeArea  = this.importContent;
    
    this.storeareaService.import(data).subscribe((response: any)=>{
