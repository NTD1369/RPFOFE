...
import { BOMViewModel, BOMDataImport } from 'src/app/_models/viewmodel/BOMViewModel';
...
@Component({
  selector: 'app-management-import-customergroup',
  templateUrl: './management-import-customergroup.component.html',
  styleUrls: ['./management-import-customergroup.component.scss']
})
export class ManagementImportCustomergroupComponent implements OnInit {

  
  value: any[] = [];
  importContent: MCustomerGroup[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private customerGroupService: CustomergroupService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }

  downloadTemplate()
  {
    this.commonService.download('M_CustomerGroup.xlsx');
  }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MCustomerGroup());
  ...
  onImportData()
  {
    ...
    data.customerGroup = this.importContent;
    
    this.customerGroupService.import(data).subscribe((response: any)=>{
