...
@Component({
  selector: 'app-management-import-merchandise',
  templateUrl: './management-import-merchandise.component.html',
  styleUrls: ['./management-import-merchandise.component.scss']
})
export class ManagementImportMerchandiseComponent implements OnInit {


  value: any[] = [];
  importContent: MMerchandiseCategory[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private merchandiseCategoryService: Merchandise_categoryService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  
  ngOnInit() {
  }
  downloadTemplate()
  {
    this.commonService.download('M_Merchandise.xlsx');
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MMerchandiseCategory());
      ...
      this.importContent = importedDataHeader.map(arr => {
        ...
        return  <MMerchandiseCategory>obj;
      })
  ...
  onImportData()
  {
    ...
    data.merchandise  = this.importContent;
    
    this.merchandiseCategoryService.import(data).subscribe((response: any)=>{
