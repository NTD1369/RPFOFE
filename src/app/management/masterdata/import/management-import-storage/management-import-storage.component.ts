...
@Component({
  selector: 'app-management-import-storage',
  templateUrl: './management-import-storage.component.html',
  styleUrls: ['./management-import-storage.component.scss']
})
export class ManagementImportStorageComponent implements OnInit {

  value: any[] = [];
  importContent: MStorage[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private storageService: StorageService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_Storage.xlsx');
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MStorage());
        ...
        for (let i = 0; i < excelHeader.length; i++) { 
          for(let j = 0; j < header.length; j++)
          {
            ...
            // debugger;
             ...
             {
                if(header[j].toLowerCase()==="isNegative" || header[j].toLowerCase()==="isnegative")
                {
  ...
  onImportData()
  {
    ...
    data.storage  = this.importContent;
    
    this.storageService.import(data).subscribe((response: any)=>{
