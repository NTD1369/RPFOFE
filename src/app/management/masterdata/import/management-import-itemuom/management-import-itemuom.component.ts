...
@Component({
  selector: 'app-management-import-itemuom',
  templateUrl: './management-import-itemuom.component.html',
  styleUrls: ['./management-import-itemuom.component.scss']
})
export class ManagementImportItemuomComponent implements OnInit {

 
  value: any[] = [];
  importContent: MItemUom[] = [];
  isResult= false;
  constructor(private commonService: CommonService ,private itemUomService: ItemuomService, private excelSrv: ExcelService, private alertifyService: AlertifyService, private route: ActivatedRoute, private router: Router) { }
  downloadTemplate()
  {
    this.commonService.download('M_ItemUoM.xlsx');
  }
  ngOnInit() {
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      const header: string[] = Object.getOwnPropertyNames(new MItemUom());
  ...
  onImportData()
  {
    ...
    data.itemUom  = this.importContent;
    
    this.itemUomService.import(data).subscribe((response: any)=>{
